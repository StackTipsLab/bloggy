  ...
  <div class="form-check bloggify-form--label"
    ...
    <label class="form-check-label">
      <input type="radio" class="form-check-input ml-0"
             v-model="$parent.radioButtonAnswer"
    ...
    <div class="check-icon --danger" v-if="questionStatus === QuestionState.ANSWERED && $parent.radioButtonAnswer === answerOption.key && !correctAnswer.includes(answerOption.key)">
    ...
    <div class="check-icon --success" v-if="questionStatus === QuestionState.ANSWERED && correctAnswer.includes(answerOption.key)">
      <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000">
        <path d="M0 0h24v24H0z" fill="none"/>
...
export default {
  ...
  methods: {
    answerClicked() {
      // this.$parent.radioButtonAnswer = this.radioButtonAnswer;
